...
\]
\begin{proof}
	נגדיר את הפונקציה $F(x, t) = A(t) x + g(t)$, בהתאם $x'(t) = F(x(t), t)$.
	לכל $x, y \in I$ ו־$t \in I$ מתקיים,
	\[
		\lVert F(x, t) - F(y, t) \rVert
		= \lVert A(t) x + g(t) - A(t) y - g(t) \rVert
		= \lVert A(t) \rVert \cdot \lVert x - y \rVert
	\]
	ומרציפות $A$ נסיק ש־$F$ מקיימת את התנאי של הגרסה למשוואות לא אוטונומיות של משפט פיקארד שהוכחנו במטלה הקודמת, בהתאם קיים פתרון יחיד $x : (T_-, T_+) \to \RR^n$ כאשר $0 \in (T_-, T_+)$ וזהו פתרון מקסימלי.

	ממשפט הטריכוטומיה והעובדה של־$\RR^n$ אין שפה נסיק שאו ש־$\limsup_{t \to T_+} \lVert x(t) \rVert = \infty$ או ש־$T_+ = \sup I$. \\
	נניח בשלילה ש־$\limsup_{t \to T_+} \lVert x(t) \rVert = \infty$, ותהי סדרה ${\{ t_n \}}_{n = 1}^\infty \subseteq I$ המקיימת $t_n \to T_+, \lVert x(t_n) \rVert \to \infty$.
	\[
		x(t)
		= x_0 + \int_{0}^{t} A(s) x(s) + g(s)\ ds
	\]
	בקטע $[0, T_+] \subseteq I$ מרציפות $A, g$ חסומות שתיהן על־ידי $K > 0$ כלשהו.
	נקבל בהתאם,
	\[
		\lVert x(t) \rVert
		\le \lVert x_0 \rVert + \int_{0}^{t} \lVert A(t) \rVert \cdot \lVert x(s) \rVert + \lVert g(s) \rVert\ ds
		\le \lVert x_0 \rVert + K T_+ + K \int_{0}^{t} \lVert x(s) \rVert\ ds
	\]
	נגזור,
	\[
		\lVert x(t) \rVert'
		\le K \lVert x(s) \rVert
	\]
	ונשתמש באי־שוויון גרונוול שהוכחנו בתרגיל הקודם,
	\[
		\lVert x(t) \rVert
		\le \lVert x(0) \rVert \cdot \exp\left(\int_{0}^{t} K\ ds\right)
		\le \lVert x_0 \rVert e^{K t}
	\]
	מצאנו ש־$x(t)$ חסומה לכל $t \ge 0$ ובהתאם $T_+ = \sup I$ בלבד.
	הטיעון עבור $T_-$ זהה.
\end{proof}
...
\begin{proof}
	נגדיר את הפונקציה $l(t) = \int_{0}^{t} \lVert F(x(s)) \rVert$ ולכן $l'(t) = \lVert F(x(t)) \rVert$ ומהנתון,
	\[
		l'(t)
		\le C(\lVert x(t) \rVert + 1)
	\]
	משוויון $(1)$ נקבל,
	\[
		x(t)
		= x_0 + \int_{0}^{t} F(x(s))\ ds
	\]
	ולכן,
	\[
		\lVert x(t) \rVert
		\le \lVert x_0 \rVert + \int_{0}^{t} \lVert F(x(s)) \rVert\ ds
		= \lVert x_0 \rVert + l(t)
	\]
	אם כך נובע,
	\[
		l'(t)
		\le C(\lVert x_0 \rVert + l(t) + 1)
		\iff l'(t) - C l(t) \le C(\lVert x_0 \rVert + 1)
	\]
	נכפיל ב־$e^{-Ct}$ ונקבל,
	\[
		e^{-Ct} l'(t) - C e^{-Ct} l(t)
		\le e^{-Ct} C (\lVert x_0 \rVert + 1)
	\]
	אבל $(e^{-CT} l(t))' = e^{-Ct} l'(t) - C e^{-Ct} l(t)$ ולכן,
	\[
		(e^{-CT} l(t))'
		\le e^{-Ct} C (\lVert x_0 \rVert + 1)
	\]
	...
	\[
		e^{-CT} l(t)
		\le \frac{1}{-C} (e^{-Ct} - 1) C (\lVert x_0 \rVert + 1)
		\iff l(t) \le C (e^{Ct} - 1) (\lVert x_0 \rVert + 1)
	\]
	מהגדרתה $0 \le l(t)$ עבור $t \ge 0$ ולכן קיבלנו שלכל $t \ge 0$ $l(t)$ חסומה, ומאי־שוויון משולש בהתאם גם $x(t)$ חסומה אף היא, בפרט מוגדרת לכל $t \ge 0$.
	משיקול סימטרי נוכל לקבל שגם לכל $t \le 0$ הפונקציה $x$ חסומה ובפרט מוגדרת ולכן $x$ מוגדרת לכל $t \in \RR$.
\end{proof}
