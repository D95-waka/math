...
\begin{proof}
	נניח ש־$z$ שורש של $f$, כלומר,
	\[
		0
		= z^3 + a z^2 + b z + c
	\]
	לאחר העברת אגפים נוכל לקבל,
	\[
		|c|_p
		= |z^3 + a z^2 + b z|_p
		\le \max\{ {|z|}_p^3, |a|_p {|z|}_p^2, |b|_p |z|_p \}
	\]
	ונתון $c \in p \ZZ_p \setminus p^2 \ZZ_p$ ולכן,
	\[
		p^{-2}
		< |c|_p
		\le p^{-1}
	\]
	מהלמה של גאוס אנו יכולים להסיק ש־$z \in \ZZ_p$ בלבד.
	אם $z \in p \ZZ_p$ אז נקבל ישר ש־$|c|_p \le p^{-2}$ בסתירה, ולכן $z \in \ZZ_p \setminus p \ZZ_p$ וכן $|z|_p = 1$.
	נקבל,
	\[
		p^{-2}
		< |c|_p
		\le \max\{ |a|_p, |b|_p \}
	\]
	כלומר נובע שגם $a, b \in p \ZZ_p \setminus p^2 \ZZ_p$, ועל־ידי שימוש באי־שוויון המשולש ההפוך נקבל שגם $p^{-2} \le |f(z)|_p$ בסתירה.
\end{proof}

\question{}
יהי $2 < p$ ראשוני ויהיו $b, c \in \ZZ_p$ כך ש־$b^2 - 4c \in \ZZ_p^*$.
נניח גם שקיים $w_0 \in \ZZ_p$ כך ש־$w_0^2 + b w_0 + c \equiv 0 \mod p$. \\
נראה שקיים $w \in \ZZ_p$ כך ש־$w^2 + b w + c = 0$.
\begin{proof}
	נסמן $f(x) = x^2 + b x + c$.
	כל תנאי הלמה של הנזל נתונים מלבד $f'(w_0) \not\equiv 0 \mod p$, לכן נשאר לבדוק זאת.
	נחשב,
	\[
		f'(x)
		= 2x + b
	\]
	נניח בשלילה ש־$f'(w_0) \equiv 0 \mod p$ ולכן נקבל שגם,
	\[
		0
		\equiv_p 2 w_0 + b
		\implies w_0 \equiv_p -\frac{b}{2}
	\]
	נציב ב־$f$,
	\[
		0
		\equiv_p f(w_0)
		\equiv_p \frac{b^2}{4} - \frac{b^2}{2} + c
		= -\frac{1}{4} (b^2 - 4c)
	\]
	אבל $| -\frac{1}{4} |_p = 1$ ולכן נקבל ש־$|b^2 - 4c|_p = 0$ בסתירה לנתון ש־$b^2 - 4c \in \ZZ_p^*$.
\end{proof}

\question{}
יהי $p$ ראשוני, נראה שקיימת תת־קבוצה $R \subseteq \ZZ_p$ עם $p - 1$ איברים כך שלכל $a \in R$ מתקיים $a^{p - 1} = 1$ וגם לכל $a, b \in R$ מתקיים $a b \in R$.
\begin{proof}
	נגדיר את הפולינום $f(x) = x^{p - 1} - 1$, אז גם $f'(x) = (p - 1) x$.
	נבחין שעבור $x = 1$ מתקיים $f(1) = 0$ וכן $f'(1) = p - 1 \not\equiv 0 \mod p$, מהלמה של הנזל נובע שקיים $w \in \ZZ_p$ המקיים $f(w) = 0$ וכן $w \equiv 1 \mod p$.
	נסמן $R = \langle w \rangle$, כלומר כלל החזקות של $w$.
	אנו יודעים כי $w^{p - 1} = 1 \implies w^p = w$ ולכן $|R| = p - 1$ בדיוק.
	נבחין כי גם לכל $a \in R$ קיים $k \in [p]$ כך ש־$a = w^k$ ולכן,
	\[
		a^{p - 1}
		= {(w^k)}^{p - 1}
		= w^{k(p - 1)}
		...
		= 1
	\]
	ולבסוף לכל $a, b \in R$ קיימים $k, l \in [p]$ כך ש־$a = w^k, b = w^l$ ולכן,
	\[
		a b
		= w^k w^l
		= w^{k + l}
		\in R
	\]
	ומצאנו כי $R$ מקיימת את כל התכונות.
\end{proof}

\question{}
עבור $p$ ראשוני נראה שמתקיים,
\[
	\ZZ_p^*
	\cong \FF_p^* \times (1 + p \ZZ_p)
\]
\begin{proof}
	נגדיר את הפונקציה $f : \FF_p^* \times (1 + p \ZZ_p) \to \ZZ_p^*$ על־ידי,
	\[
		f(z, x)
		= z x
	\]
	ברור כי זהו הומומורפיזם.
	ניזכר כי $y \in \ZZ_p^*$ אם ורק אם קיימת סדרה ${(a_n)}_{n = 0}^{\infty} \subseteq [p]$ המקיימת,
	\[
		y
		= \sum_{n = 0}^\infty a_n p^n
	\]
	בנורמה ה־$p$־אדית.
	נניח ש־$f(z, x) = f(z', x') = y$ ונקבל ש־$a_1 = z = z'$ בלבד, שכן $|z|_p = 1$ ו־$|x - 1|_p < 1$.
	הנחנו ש־$z x = z' x'$ ומצאנו ש־$z = z'$ ולכן נובע ש־$x = x'$ וקיבלנו ש־$f$ הוא הומומורפיזם חד־חד ערכי. \\
	נעבור להראות שהוא על, כלומר נמצא $z, x$ עבורם $f(z, x) = y$ לערך $y \in \ZZ_p^*$ שרירותי.
	באופן דומה לחד־חד ערכיות נגדיר $z = a_0$ ולכן $x = \frac{y}{z}$ מוגדר, אבל נרצה להראות שגם $x \in 1 + p \ZZ_p^*$.
	נבחין כי המקדמים בפיתוח ה־$p$־אדי של $x$ הם בדיוק $b_n = \frac{a_n}{z}$, לכן $b_0 = \frac{a_1}{z} = \frac{a_1}{a_1} = 1$.
	אם כך אז $x - 1$ הוא מספר שהמקדמים שלו מקיימים בדיוק $c_0 = 0$, כלומר $x - 1 \in \ZZ_p^*$.
	נסיק ש־$f$ איזומורפיזם.
\end{proof}

\question{}
נניח ש־$p > 2$ ראשוני ונראה שמתקיים,
\[
	{(1 + p \ZZ_p)}^2
	= (1 + p \ZZ_p)	
\]
\begin{proof}
	...
	\[
		...
		\subseteq (1 + p \ZZ_p)	
	\]
	ולכן נותר להראות את ההכלה ההפוכה.
	יהי $a \in (1 + p \ZZ_p)$, אז מהגדרתו קיים $z \in p \ZZ_p$ המקיים,
	\[
		a
		= 1 + z
		\equiv 1 + 0 \mod p
	\]
	ולכן כמסקנה מהכיתה קיים $w \in \ZZ_p$ כך ש־$w^2 = a$ ולכן $a \in {(1 + p \ZZ_p)}^2$.
\end{proof}
