...
def question_1(examples: np.ndarray, predictions: list[float]) -> np.ndarray:
...
if __name__ == "__main__":
    ...
    for P in [5, 20, 30, 50, 100, 150, 200, 500]:
        ...
        for i in range(M):
            ...
            sum += np.rad2deg(np.arccos((weights.transpose() @ optimal_weights) / (np.linalg.norm(weights) * K)))

        sum /= M
        data.append([P, sum[0][0]])

    data = np.transpose(data)
    fig, ax = plt.subplots(layout='constrained')
    bars = ax.bar([f"P={x}" for x in data[0]], data[1])
    ax.bar_label(bars, labels = [f"{x:.4f}" for x in data[1]], padding=3)
    plt.savefig('bin/out1_4.jpg')
