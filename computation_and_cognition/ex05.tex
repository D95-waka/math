...
\begin{solution}
	...
	\[
		e_i
		= -x_i (1 - y) + x_i \frac{\exp(- w^t x)}{1 + \exp(- w^t x)}
	\]
\end{solution}
...
\begin{solution}
	...
	\[
		\Delta w_i
		= \eta R \left(-x_i (1 - y) + \frac{x_i \exp(- w^t x)}{1 + \exp(- w^t x)}\right)
	\]
	נשים לב שלצורך כלל העדכון אנו צריכים את התוצאה, את $R$ ואת תוצאת כלל הנוירונים באותה שכבה, ולכן הוא לא מקומי.
\end{solution}

\question{}
נשתמש ב־dataset נתון כדי לבנות פרספטרון בינארי ולאמנו על־פי אלגוריתם ונתוני השאלה הקודמת כאשר אנו רצים על מידע של תמונות של הספרה 0 והספרה 1.
נשתמש ב־reinforce כאשר אנו רצים פעם יחידה על כל דוגמה וכאשר $\eta = 0.001$.
נשתמש גם בפונקציית הגמול $R(y) = \indicator_{\{ c \}}$ עבור $c$ התיוג.

\subquestion[3]
כל 51 אימונים נתעד את רמת הדיוק של המודל על־ידי בדיקת ההצלחה הממוצעת שלו מול מאגר בדיקה עליו אנו לא מאמנים את המודל, ונציג זאת בגרף.
\begin{center}
	\includegraphics[width=9cm]{./bin/out5_2}
\end{center}
מסיבה לא ממש ברורה לא משנה באיזה אופן בדקתי מול מאגר הבדיקה קיבלתי תוצאה לא יציבה ושנעה סביב $50\%$ הצלחה, לכן לקחתי את 2000 הערכים האחרונים במידע לאימון והעברתי אותם החוצה למאגר בדיקות שני (עליו גם לא התבצע אימון).
הפעם הגרף נראה כך,
\begin{center}
	\includegraphics[width=9cm]{./bin/out5_1}
\end{center}
הפעם אנו יכולים הליך למידה מהיר ושתואם את האלגוריתם הנבחר, כולל הקפיצות בסוף שממוצעיהן שואף להצלחה מלאה בתיוג.

\subquestion{}
נציג את וקטור המשקולות שמתקבל לאחר הלמידה כתמונה,
\begin{center}
	\includegraphics[width=12cm]{./bin/out5_3}
\end{center}
נסביר תוצאה זו.
ישנם שני צברים נראים לעין, אחד הוא בצורה הכללית של המספר 0, ואילו הצבר השני הוא בצורה של הספרה 1, זאת בהתאם לצורה הנלמדת של המידע, והצלחתו בהפרדה בין שתי הספרות.
למעשה תמונה זו ממחישה לנו מהו ההבדל המהותי בין הספרות, שכן אכן אחת עגולה והשנייה היא קו.
