...
\begin{solution}
	מתקיים $y(\bar{x}) = H(w_1 \cdot 7) = 0$ ולכן נסיק $w_1 < 0$, בהתאם נובע $y(\bar{x}) = H(5 \cdot w_1) = 0$ (תשובה ב').
\end{solution}
...
\begin{solution}
	נניח ש־$\bar{w} \cdot \bar{x} = \alpha \bar{w} \cdot \bar{z}$ עבור $\alpha < 0$.
	במקרה זה מתקיים,
	\[
		y(\bar{x}) = \varepsilon
		\iff H(\bar{w} \cdot \bar{x})
		\iff \indicator_{(0, \infty)}(\bar{w} \cdot \bar{x}) = \varepsilon
		\iff \indicator_{(0, \infty)}(\alpha \bar{w} \cdot \bar{x}) \ne \varepsilon
		\iff \indicator_{(0, \infty)}(\bar{w} \cdot \bar{z}) \ne \varepsilon
		\iff y(\bar{z}) \ne \varepsilon
	\]
	כלומר נקבל שהתוצאות בהכרח שונות (תשובה ג').
\end{solution}
...
\question{}
בשאלה זו נציג את התוצאות של הרצה של מבחן ממוחשב שבו מוגרלות $P$ דוגמות לנקודות בתחום ${[-10, 10]}^2$, עבור כל נקודה נגדיר את הסיווג $y = x_1 > x_2$.
המבחן הממוחשב מקבל את הנקודות והסיווגים ועליו לאמן פרספטרון בינארי מתאים.

\subquestion[3]
עבור $P = 1000$ נציג את כלל הנקודות שהצגנו לצד הפרספטרון המאומן, נציג את וקטור המשקולות ואת הקו המגדיר אותו. \\
\includegraphics[width=9cm]{bin/out1_3}

\subquestion[4]
עבור $P \in \{ 5, 20, 30, 50, 100, 150, 200, 500 \}$ דוגמות נריץ את המבחן $M = 100$ ונחשב את השגיאה הממוצעת של הפרספטרון שאומן. \\
\includegraphics[width=9cm]{bin/out1_4} \\
נבחין שככל שיש יותר דוגמות ככה השגיאה הממוצעת יורדת ושואפת לאפס.
