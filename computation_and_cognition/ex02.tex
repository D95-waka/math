...
\question{}
יהי פרספטרון לינארי עם סף המנסה ללמוד את הפונקציה $Y_0 = X^3 - X^2$ כאשר $Y_0 \sim U([-1, 1])$.
נניח שלפרספטרון שני קלטים, $\vec{x} = (x, 1)$, ובהתאם מתקבל $y = w_1 x + w_2$.
...
\begin{solution}
	...
	\[
		...
		= w_2 + \frac{1}{3}
	\]
	ממשפט כופלי לגרנז' נובע ישירות ש־$\vec{w} = {(\frac{3}{5}, -\frac{1}{3})}^t$ הוא וקטור המשקולות בו $\varepsilon_g(\vec{w})$ מינימלי.
\end{solution}
...
\subquestion[2]
בהרצה לדוגמה של קוד המבחן התקבל הווקטור $\vec{w} = {(0.568, -0.33)}^t$, \\
...
בגרף המצורף, נראית תוצאה של ניסוי זה.
