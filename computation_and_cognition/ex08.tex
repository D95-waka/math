...
\begin{proof}
	...
	\[
		...
		\iff \sigma (\ln X_s - \ln X_g) = - {(- \ln p)}^\alpha
		\iff -\sigma \ln \frac{X_s}{X_g} = {(- \ln p)}^\alpha
	\]
	ולבסוף על־ידי עוד $\ln$,
	\[
		\ln(-\sigma \ln \frac{X_s}{X_g}) = \ln {(- \ln p)}^\alpha
		\iff \ln \sigma + \ln(- \ln \frac{X_s}{X_g}) = \alpha \ln(- \ln p)
	\]
	וקיבלנו את התוצאה הרצויה.
\end{proof}

\subquestion[5]
נציג גרף של $\sigma, \alpha$ באיטרציה השנייה כפונקציה של ערכם באיטרציה הראשונה ונחקור אותו.
\begin{solution}
	נציג את הגרף,
	\begin{center}
		\includegraphics[width=9cm]{bin/out8_4}
	\end{center}
	נוסף קו המגמה $y = x$.
	נראה שאנשים לא נוטים לשנות את דעתם בין מהלכים שונים.
\end{solution}
