...
\end{proof}
\begin{conclusion}[אי־שוויון בסל]
	לכל $x \in V$,
	\[
		{\lVert x \rVert}^2
...
\end{conclusion}
\begin{conclusion}[אי־שוויון שוורץ]
...
\begin{proof}
	...
	כלומר,
	\[
		{\lVert f - \sum_{n = 0}^{2N} \langle e_n, f \rangle e_n \rVert}^2
		\xrightarrow{N \to \infty} 0
	\]
	אבל,
	\[
...
\begin{theorem}[התכנסות נקודתית של טורי פורייה]
	נניח ש־$f : \RR \to \RR$ מחזורית עם מחור $2 \pi$ ובאינטגרבילית רימן.
	...
	\]
	לכל $u \in (0, \delta)$.
	כאשר גם $f(x_0 + 0) = \lim_{v \to 0} f(x_0 + v)$ וכן $f(x_0 - 0) = \lim_{v \to 0} f(x_0 - v)$.
	מתקיים,
	\[
...
\begin{proof}
	...
	\[
		S_N(x_0)
		= \lim_{N \to \infty} \int_{0}^{\pi} (f(x_0 + u) - f(x_0 + 0) + f(x_0 - u) - f(x_) - 0) \frac{\sin((N + \frac{1}{2}) u)}{2 \sin \frac{u}{2}}\ du
	\]
...
\begin{example}
	...
	ונבחר,
	\[
		U = \{ f \in C[0, 1] \mid \int_{0}^{1} f(x)\ dx = 0, f(1) = 0 \}
	\]
...
\begin{example}
	...
	\[
		...
		= r
	\]
	ולכן נובע $t \varphi + (1 - t) \psi \in B(r, f)$.
\end{example}
...
\begin{theorem}[תנאי מספיק לקבלת מרחק במרחבים שלמים]
	יהי $(V, \langle \cdot, \cdot \rangle)$ מרחב מכפלה פנימית, כך שאם $\lVert v \rVert = {(\langle v, v \rangle)}^{1 / 2}$ אז $(V, \lVert \cdot \rVert)$ מרחב שלם.
...
\begin{definition}[הטלה אורתוגונלית למרחב הילברט]
	...
	נגדיר $P_U : V \to U$ על־ידי $P_U(f) = g$ עבור $\dist(f, U) = g$.
\end{definition}
\begin{remark}
	מתקיים,
	\[
...
\begin{proof}
	...
	\[
		\alpha {\left\lvert \langle g, f - P_U(f) \right\rvert}^2
		\le 2 {\left\lvert \langle g, f - P_U(f) \right\rvert}^2 {\lVert g \rVert}^2
	\]
	לכל $\alpha > 0$, כאשר אי־שוויון זה יכול להתקיים רק כאשר $\langle g, f - P_U(f) \rangle = 0$.

	\textbf{2}
	נשים לב כי אם $h \in U$ ומתקיים $\langle f - h, g \rangle = 0$ לכל $g \in U$ אז גם $\langle f - h \rangle = 0$.
	בהתאם,
	\[
		{\lVert f - h \rVert}^2
	...
	\[
		...
		= \langle f - P_U(f), f \rangle - \langle f - P_U(f), P_U(f)
		= \langle f - P_U(f), f \rangle - \langle f - 0
	\]
...
\begin{definition}[תת־מרחב אורתוגונלי]
	יהי $(V, \langle \cdot, \cdot \rangle)$ מרחב מכפלה פנימית, ויהי תת־מרחב $U \subseteq V$.
