...
\begin{proof}
	It is sufficient to show that $\val(\lnot X)$ is recursive by the last part.

	In the Tachles we can enumerate finite structures and by that get if $\varphi \in \sat_{< \infty}(X)$, and then use union.

	We also need to show that $\lnot \sat_{< \infty}(X) = \sat_{< \infty}(\lnot X)$ is recursive, I think it's done the same way.
\end{proof}
