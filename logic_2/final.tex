...
\begin{proof}
	By the lemmas before the first incompleteness theorem we can choose some $\Pi_2$-sentence $\varphi$ such that $\hat{\NN} \models \varphi$ but $\PA \not\vdash \varphi$.
	Let us assume that $\varphi = \forall x \exists y\ \psi$ for $\psi \in \Sigma_0^0$ by the definition of $\Pi_2$, and define,
	\[
		f(x) = y
		\iff \exists y\ \psi
	\]
	There is the problem that $\psi$ is not necessarily fulfills the function sentence, namely,
	\[
		\forall x \exists y (\psi(x, y) \land (\forall y'\ \psi(x, y') \to y = y'))
	\]
	But by using minimization we can define,
	\[
		\psi' = \psi y\ \psi(x, y)
	\]
	such that $\psi$ now doe define a function $f$, and as a $\Sigma_1^0$-formula this function is also recursive.
	By $\hat{\NN} \models \varphi'$ for modified version of $\varphi$ for $\psi'$, we can also deduce that $f$ is total, but,
	\[
		\PA \not\vdash \forall x \exists y\ \psi'(x, y)
	\]
	as required.

	Let us assume towards a contradiction that there is $\phi(x, y) \in \Sigma_1$ such that,
	\[
		\hat{\NN} \models \forall x \forall y\ \phi(x, y) \leftrightarrow \psi(x, y)
	\]
	and that $\PA \vdash \forall x \exists y\ \phi(x, y)$.
	We can use induction scheme on $x$ with $\exists y\ \phi(x, y) \land \psi(x, y)$, deducing using the function sentence that $\PA \vdash \psi \leftrightarrow \phi$, and therefore,
	\[
		\PA \cup \{ \phi \} \vdash \forall x \exists y\ \psi(x, y)
	\]
	By the contradiction assumption it follows that,
	\[
		\PA
		\vdash \forall x \exists t\ \psi(x, y)
	\]
