...
\begin{solution}
	We will find total recursive function $f : \NN \to \NN$ such that for any $\Sigma_1$-formula $\varphi(x, y)$ such that,
	\[
		\hat{\NN} \models \varphi(x, y)
		\iff f(x) = y
	\]
	it holds that $\PA \not\vdash \forall x \exists y\ \varphi(x, y)$.

	In class we defined a variant of Ackerman function $B : \NN^2 \to \NN$.
	By our definition for $B$ we get that it is defined for any numerator, in particular $B^{\hat{\NN}}$ is a total function as required.
	$B$ was also proved to not be primitive-recursive.

	Let $\varphi \in \sent_{L_{\PA}}$ be a sentence such that,
	\[
		\hat{\NN} \models B(x, y) = z
		\iff \hat{\NN} \models \varphi(x, y, z)
	\]
	We will show that,
	\[
		\PA \not\vdash \forall x \forall y \exists z\ \varphi(x, y, z)
	\]

	By the completeness theorem it suffices to find a model $\Nn \models \PA$ such that,
	\[
		\Nn \models \lnot \forall x \forall y \exists z\ \varphi(x, y, z)
		\iff
		\Nn \models \exists x \exists y \forall z\ \lnot \varphi(x, y, z)
	\]
	This is the claim that $B$ is not total in $\Nn$.

	Consider the non-standard model $\Nn \models \PA$ that as for $\langle N, \le^\Nn \rangle$ it holds that,
	\[
		\Nn
		\cong \NN + \QQ \times \ZZ
	\]
	$\varphi \in \Sigma_1$ therefore $\varphi = \exists v\ \psi(v, x, y, z)$ for $\psi \in \Sigma_0$.
	% and let us define the chain $C = \{ (1, 1, n) \mid n \in \ZZ \}$.
	% Notate $\alpha \in C$ and observe $B(\alpha, \alpha)$, if $B$ is total and $\beta = B(\alpha, \alpha)$, we will show that for any $a \in \Nn$, $a <^\Nn \beta$ in induction.
	% For the case of $0$ the claim is trivial by $\alpha$'s definition.
	% If $a <^\Nn \beta$ then $S(a) <^\Nn S(\beta) <^\Nn B(S(\alpha), \alpha) = B(\alpha, \alpha) = \beta$.
\end{solution}
