...
\begin{proof}
	By defining the recursive function,
	\[
		H(0) = 0,
		H(n + 1) = S(H(n))
	\]
	we deduce that the formula $\varphi(x, y) = \exists \underline{n}\ y = \underline{n} \land n = x$ defines recursive function such that for numerals it is identity.
	We also know that,
	\[
		\hat{\NN}
		\models \forall x \exists y\ \varphi(x, y)
	\]
	namely, in the standard model every element is numeral.
	Then by completeness,
	\[
		\hat{\NN}
		\vdash \forall x \exists y\ \varphi(x, y)
	\]
	If by contradiction,
	\[
		\PA
		\vdash \forall x \exists y\ \varphi(x, y)
	\]
	then for a non-standard model $\Nn \models \PA$ and a non-standard element $c \in N$,
	\[
		\Nn
		\vdash \varphi(c)
	\]
	meaning that $c$ is standard, a contradiction.

	We found total recursive function $f : \NN \to \NN$ such that for any non-standard model it is not total,
	then for every such $\varphi(x, y)$ that defines $f$ it is implied that $\PA \not\vdash \forall x \exists y\ \varphi(x, y)$.
\end{proof}
