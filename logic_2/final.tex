...
\begin{proof}
	...
	Consider $\Mm = \langle \bigcup A, = \rangle$, we define $\Nn = \Mm^A / U$, and the sentence $\varphi = \exists x, y,\ x \ne y$.
	By Łoś theorem and the fact that $a \in A$ is not empty for all $a$, we get that $\Mm \models \varphi$ and therefore $\Nn \models \varphi$.
	It follows that there are $[f], [g] \in N$ such that $[f] \ne [g]$, and $f, g : A \to \bigcup A$.
	\[
		\{ j \in A \mid f(j) = g(j) \} \notin S
		\iff \{ j \in A \mid f(j) \ne g(j) \} \in S
	.\]

	Maybe $\Mm = \langle A \cup \bigcup A; \in \rangle$ such that $x \in y \iff x \in y \in A$.
	Also $\varphi = \forall x (\exists y,\ y \in x \lor \exists y, z,\ y, z \in x)$.
	Of course $\Mm \models \varphi$, therefore $\Mm^M / U \models \varphi$.

	We need to show that $S$ contains the empty set somehow.
	Let $f : B \to \bigcup B$ be some choice function for $B \notin U$, and let us extend it to $A \to \bigcup A$ function by setting $\forall x \notin B,\ f(x) = c$ for some constant $c \in \bigcup A$.
	We can define $\varphi = \forall x, x \ne c$ by creating constant symbol in $L$.
	It's clear that $[f] \in \Nn$ and $\{ i \in A \mid f(i) = c \} \in S$
	\[
		\Mm \not\models \varphi
		\iff \Nn \not\models \varphi
		\iff \{ i \in A \mid \Mm \models \varphi \} \in S
	.\]

	We take some $\varphi = \exists x = c$ for constant then it holds and we get $x = [f]$ for $\{ i \in A \mid f(i) = c \} \in S$ and define $f' \in [f]$ such that $f'(i) \ne c$ for certain values.
