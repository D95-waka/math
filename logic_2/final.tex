...
\begin{proof}
	...
	\[
		\Gamma_s
		= \{ R_i(x_0, \ldots, x_{s - 1}) \mid i < n, k_i \le s \}
	\]
	$n$ is finite and therefore $s$ and $\Gamma_s$ as well.
	We say that an $L$-structure $\Mm$ with $M \subseteq \NN$ is $\Gamma$-homogeneous if for every $R_i(x_0, \ldots, x_{s - 1}) \in \Gamma_s$,
	\[
		\forall a_0 < \cdots < a_{s - 1}, b_0 < \cdots < b_{s - 1} \in M,\ 
		\Mm \models R_i(a_0, \ldots, a_{s - 1})
		\iff \Mm \models R_i(b_0, \ldots, b_{s - 1})
	.\]
	% TODO: I think the way this hint is written is misleading, and that there should not be such `s`
\end{proof}
