...
\begin{proof}
	We will construct an embedding by recursion.
	Let $f_0 = \{ \langle d^\Nn, d^\Mm \rangle \mid d \in L, d \text{ is constant symbol} \}$, this is an embedding by $T$'s completeness.

	Let $f_{\delta} : A \to B$ be a partial embedding, $A \subseteq N, B \subseteq M$, $A = {\{ a_i \}}_{i < \delta}$.
	Let $a \in N \setminus A$ and let $p = tp(a / N_A)$	be the complete type such that $\Nn \models p(a)$.
	Then $q = p_{f(a_0), \ldots}^{a_0, \ldots}$ is a type $\in S_1(B)$, it is consistent by compactness and complete as closure under inference.
	...
	Then $f_{\delta} = \bigcup_{\alpha < \delta} f_{\alpha}$ is defined and acts as embedding as any term or formula are of finite length and thus embedded correctly in some $\alpha < \delta$.

	We get that $f_{\alpha}$ exists for each $\alpha \le \kappa$, note that by the definition of $\kappa$-saturation this process cannot be extended after reaching $\kappa$.
