...
\begin{solution}
	...
	נניח עתה ש־$\forall x \in \RR,\ |f^{-1}(\{ x \})| = \aleph_0$.
	נסמן $E = f^{-1}([0, 1])$, אז נטען ש־$|E| = |\RR|$.
	אחרת נוכל להגדיר סדרת $E_n = f^{-1}([n, n + 1])$ עבור $n \in \ZZ$, אם $|E_n| = |\RR|$ לאיזשהו $n$ נבחר אותו, אחרת נקבל שמתקיים $\RR = \bigcup_{n \in \ZZ} E_n$ אבל $|f^{-1}(\RR)| = \aleph_0$ וזו סתירה.
	אז נובע ש־$|E| = |\RR|$, ונגדיר סדרת קטעים $E = F_1 \supsetneq F_2 \supsetneq \cdots$ על־ידי חציית הקטעים ושימוש בטיעון דומה, ולבסוף מסגירות לחיתוך בן־מניה נקבל שקיים $a \in \RR$ כך ש־$|f^{-1}(\{ a \})| = |\RR|$ בסתירה.
