...
\begin{solution}
	נחשב את עקמומיות גאוס תוך שימוש בנוסחה שהוצגה והתבנית השנייה,
	\[
		K
		= \frac{\det I}{\det I\!I}
	...
	.\]
	נחשב את העקמומיות הממוצעת באופן דומה,
	\[
...
\begin{proof}
	יהי משטח $S$ ו־$f : U \to S$ פרמטריזציה מקומית לפי מרחק שלו.
	יהי עקום $c : I \to S$ המשוכן ב־$f(U)$, לכן לכל נקודה בו מתקיים $\kappa = \kappa_n = 0$ מההנחה.
	אבל $\kappa = \lVert c'' \rVert$ ולכן $c'' \equiv 0$ ולכן $c' \equiv \alpha$ קבועה כך ש־$\alpha \in S^2$, שהרי $c$ היא לפי אורך.
	בהתאם קיבלנו ש־$c(t) = P + \alpha (t - t_0)$ עבור $c(t_0) = P \in f(U) \subseteq S$ ו־$t_0 \in I$.
	כלומר קיבלנו ש־$c(I)$ ישר ולכן משאלה 1 סעיף ב' נקבל שמקומית $S$ היא תת־יריעה אפינית ממימד 2, כלומר $S$ משוכנת במישור.
\end{proof}
